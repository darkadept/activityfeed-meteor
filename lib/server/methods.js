...
var parameterFunctions = {
	...
	self: function(id, name) {
		if (Meteor.userId() === id) return 'You';
		return name;
	}
};
...
function addActivityItem(subject, object, verb, id) {
	return function(done) {
		...
		var parameterObject = _.extend({}, parameterFunctions, {
			subject: subjectDoc,
			object: objectDoc
		});

		// Pass the parameter object through the summary, extra, and determineUsers template functions
